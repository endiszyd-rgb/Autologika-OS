...
ipcMain.handle('jobParts:update',(_,{id,data})=>{const db=getDb();let price=+data.unit_price||0;if(!price&&+data.unit_cost>0)price=Math.round((+data.unit_cost)*(1+partMarkup(+data.unit_cost))*100)/100;const supplierName=data.supplier_name||(data.supplier_id?db.prepare('SELECT name FROM suppliers WHERE id=?').get(data.supplier_id)?.name:'')||'';db.prepare(`UPDATE job_part_orders SET supplier_id=?,supplier_name=?,part_no=?,oe_number=?,inventory_part_id=?,barcode=?,brand=?,vehicle_fitment=?,cross_numbers=?,lookup_source=?,lookup_url=?,name=?,qty=?,unit_cost=?,unit_price=?,external_order_no=?,expected_at=?,notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(data.supplier_id||null,supplierName,data.part_no||'',data.oe_number||'',data.inventory_part_id||null,normalizeBarcode(data.barcode||''),data.brand||'',data.vehicle_fitment||'',data.cross_numbers||'',data.lookup_source||'',data.lookup_url||'',data.name||'',+data.qty||1,+data.unit_cost||0,price,data.external_order_no||'',data.expected_at||null,data.notes||'',id);return true})
