...
const { ensureInstalledJobPart } = require('./job-part-installation.cjs')
...
ipcMain.handle('jobParts:status',(_,{id,status})=>{const db=getDb(),x=db.prepare('SELECT * FROM job_part_orders WHERE id=?').get(id);if(!x)return false;const tx=db.transaction(()=>{requireEditableOrder(db,x.order_id);db.prepare(`UPDATE job_part_orders SET status=?,updated_at=CURRENT_TIMESTAMP,ordered_at=CASE WHEN ?='ZAMOWIONE' AND ordered_at IS NULL THEN CURRENT_TIMESTAMP ELSE ordered_at END,received_at=CASE WHEN ?='ODEBRANE' THEN CURRENT_TIMESTAMP ELSE received_at END,installed_at=CASE WHEN ?='ZAMONTOWANE' THEN CURRENT_TIMESTAMP ELSE installed_at END,returned_at=CASE WHEN ?='ZWROT_ZAKONCZONY' THEN CURRENT_TIMESTAMP ELSE returned_at END WHERE id=?`).run(status,status,status,status,status,id);if(status==='ZAMONTOWANE')ensureInstalledJobPart(db,x);return true});return tx()})
ipcMain.handle('jobParts:batchStatus',(_,{ids,status,externalOrderNo='',expectedAt=null})=>{const db=getDb();const clean=[...new Set((ids||[]).map(Number).filter(Boolean))];if(!clean.length)return{ok:false,count:0};const tx=db.transaction(()=>{const placeholders=clean.map(()=>'?').join(','),parts=db.prepare(`SELECT * FROM job_part_orders WHERE id IN (${placeholders})`).all(...clean),orderIds=[...new Set(parts.map(x=>x.order_id))];for(const orderId of orderIds)requireEditableOrder(db,orderId);for(const part of parts){db.prepare(`UPDATE job_part_orders SET status=?,external_order_no=CASE WHEN ?!='' THEN ? ELSE external_order_no END,expected_at=COALESCE(?,expected_at),ordered_at=CASE WHEN ?='ZAMOWIONE' AND ordered_at IS NULL THEN CURRENT_TIMESTAMP ELSE ordered_at END,received_at=CASE WHEN ?='ODEBRANE' THEN CURRENT_TIMESTAMP ELSE received_at END,installed_at=CASE WHEN ?='ZAMONTOWANE' THEN CURRENT_TIMESTAMP ELSE installed_at END,returned_at=CASE WHEN ?='ZWROT_ZAKONCZONY' THEN CURRENT_TIMESTAMP ELSE returned_at END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(status,externalOrderNo,externalOrderNo,expectedAt,status,status,status,status,part.id);if(status==='ZAMONTOWANE')ensureInstalledJobPart(db,part)}});tx();return{ok:true,count:clean.length}})
