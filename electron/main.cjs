...
const { createOrderItem, issueInventoryPart, jobPartTechnicalDescription, removeOrderItem, requireEditableOrder, updateOrderItem, updateOrderItemDescription } = require('./inventory-usage.cjs')
...
ipcMain.handle('cloudSync:retry',async(_e,id)=>cloudSync.retryPending(id))
...
ipcMain.handle('orders:notesSave',(_,{id,data})=>{const db=getDb();requireEditableOrder(db,id);db.prepare(`INSERT INTO order_notes(order_id,intake_notes,release_notes,qc_notes,updated_at) VALUES (?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(order_id) DO UPDATE SET intake_notes=excluded.intake_notes,release_notes=excluded.release_notes,qc_notes=excluded.qc_notes,updated_at=CURRENT_TIMESTAMP`).run(id,data.intake_notes||'',data.release_notes||'',data.qc_notes||'');return true})
...
ipcMain.handle('worklog:start',(_,{orderId,note})=>{const db=getDb();try{requireEditableOrder(db,orderId)}catch(error){return{ok:false,error:error.message}}const active=db.prepare("SELECT id FROM work_logs WHERE worker='Właściciel' AND ended_at IS NULL").get();if(active)return{ok:false,error:'Najpierw zatrzymaj aktywny licznik czasu.'};const r=db.prepare('INSERT INTO work_logs(order_id,worker,note) VALUES (?,?,?)').run(orderId,'Właściciel',note||'');return{ok:true,id:r.lastInsertRowid}})
...
ipcMain.handle('quotes:get',(_,orderId)=>{
  ...
  if(!q){requireEditableOrder(db,orderId);const r=db.prepare("INSERT INTO quotes(order_id,status) VALUES (?,'ROBOCZA')").run(orderId);q=db.prepare('SELECT * FROM quotes WHERE id=?').get(r.lastInsertRowid)}
...
ipcMain.handle('quotes:requestApproval',(_,id)=>{
  ...
  requireEditableOrder(db,q.order_id)
...
ipcMain.handle('worklog:startAs',(_,{orderId,employeeId,note})=>{const db=getDb();try{requireEditableOrder(db,orderId)}catch(error){return{ok:false,error:error.message}}const emp=db.prepare('SELECT * FROM employees WHERE id=? AND active=1').get(employeeId);if(!emp)return{ok:false,error:'Nie znaleziono aktywnego pracownika.'};const active=db.prepare('SELECT id FROM work_logs WHERE employee_id=? AND ended_at IS NULL').get(employeeId);if(active)return{ok:false,error:`${emp.name} ma już aktywny licznik czasu.`};const r=db.prepare('INSERT INTO work_logs(order_id,worker,note,employee_id) VALUES (?,?,?,?)').run(orderId,emp.name,note||'',employeeId);return{ok:true,id:r.lastInsertRowid}})
...
ipcMain.handle('jobParts:list',(_,filter='OTWARTE')=>{const db=getDb();let where="";if(filter==='OTWARTE')where="WHERE j.status NOT IN ('ZAMONTOWANE','ZWROT_ZAKONCZONY','ANULOWANE')";else if(filter!=='WSZYSTKIE')where="WHERE j.status=?";const sql=`SELECT j.*,COALESCE(s.name,NULLIF(j.supplier_name,'')) supplier,o.vehicle_id,v.plate,v.make,v.model,c.name customer FROM job_part_orders j JOIN orders o ON o.id=j.order_id JOIN vehicles v ON v.id=o.vehicle_id LEFT JOIN customers c ON c.id=v.customer_id LEFT JOIN suppliers s ON s.id=j.supplier_id ${where} ORDER BY CASE j.status WHEN 'DO_ZAMOWIENIA' THEN 0 WHEN 'ZAMOWIONE' THEN 1 WHEN 'W_DRODZE' THEN 2 WHEN 'ODEBRANE' THEN 3 WHEN 'DO_ZWROTU' THEN 4 ELSE 5 END,j.created_at DESC`;return filter!=='OTWARTE'&&filter!=='WSZYSTKIE'?db.prepare(sql).all(filter):db.prepare(sql).all()})
ipcMain.handle('jobParts:forOrder',(_,orderId)=>getDb().prepare(`SELECT j.*,COALESCE(s.name,NULLIF(j.supplier_name,'')) supplier FROM job_part_orders j LEFT JOIN suppliers s ON s.id=j.supplier_id WHERE j.order_id=? ORDER BY j.created_at DESC`).all(orderId))
ipcMain.handle('jobParts:create',(_,d)=>{const db=getDb();requireEditableOrder(db,d.order_id);let price=+d.unit_price||0;if(!price)price=Math.round((+d.unit_cost||0)*(1+partMarkup(+d.unit_cost||0))*100)/100;const vehicle=db.prepare(`${orderSelect} WHERE o.id=?`).get(d.order_id);if(!vehicle)throw new Error('Wybrane zlecenie nie istnieje.');const snapshot=JSON.stringify({vehicle_id:vehicle.vehicle_id,plate:vehicle.plate||'',vin:vehicle.vin||'',make:vehicle.make||'',model:vehicle.model||'',generation:vehicle.generation||'',year:vehicle.year||'',engine:vehicle.engine||'',engine_code:vehicle.engine_code||''});const supplierName=d.supplier_name||(d.supplier_id?db.prepare('SELECT name FROM suppliers WHERE id=?').get(d.supplier_id)?.name:'')||'';const r=db.prepare(`INSERT INTO job_part_orders(order_id,supplier_id,supplier_name,part_no,oe_number,inventory_part_id,vehicle_snapshot,barcode,brand,vehicle_fitment,cross_numbers,lookup_source,lookup_url,name,qty,unit_cost,unit_price,status,external_order_no,expected_at,ordered_at,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,CASE WHEN ? IN ('ZAMOWIONE','W_DRODZE','ODEBRANE') THEN CURRENT_TIMESTAMP ELSE NULL END,?)`).run(d.order_id,d.supplier_id||null,supplierName,d.part_no||'',d.oe_number||'',d.inventory_part_id||null,snapshot,normalizeBarcode(d.barcode||''),d.brand||'',d.vehicle_fitment||'',d.cross_numbers||'',d.lookup_source||'',d.lookup_url||'',d.name,+d.qty||1,+d.unit_cost||0,price,d.status||'DO_ZAMOWIENIA',d.external_order_no||'',d.expected_at||null,d.status||'DO_ZAMOWIENIA',d.notes||'');return{id:r.lastInsertRowid,unit_price:price}})
ipcMain.handle('jobParts:remove',(_,id)=>{const db=getDb(),part=db.prepare('SELECT id,order_id,name,status FROM job_part_orders WHERE id=?').get(Number(id));if(!part)return{removed:false};const tx=db.transaction(()=>{requireEditableOrder(db,part.order_id);db.prepare('DELETE FROM job_part_orders WHERE id=?').run(part.id);db.prepare(`INSERT INTO order_events(order_id,event_type,title,details) VALUES (?,?,?,?)`).run(part.order_id,'PART_REMOVED','Usunięto część z listy',`${part.name} · ${part.status}`)});tx();return{removed:true,orderId:part.order_id,status:part.status}})
ipcMain.handle('jobParts:status',(_,{id,status})=>{const db=getDb(),x=db.prepare('SELECT * FROM job_part_orders WHERE id=?').get(id);if(!x)return false;const tx=db.transaction(()=>{requireEditableOrder(db,x.order_id);db.prepare(`UPDATE job_part_orders SET status=?,updated_at=CURRENT_TIMESTAMP,ordered_at=CASE WHEN ?='ZAMOWIONE' AND ordered_at IS NULL THEN CURRENT_TIMESTAMP ELSE ordered_at END,received_at=CASE WHEN ?='ODEBRANE' THEN CURRENT_TIMESTAMP ELSE received_at END,installed_at=CASE WHEN ?='ZAMONTOWANE' THEN CURRENT_TIMESTAMP ELSE installed_at END,returned_at=CASE WHEN ?='ZWROT_ZAKONCZONY' THEN CURRENT_TIMESTAMP ELSE returned_at END WHERE id=?`).run(status,status,status,status,status,id);if(status==='ZAMONTOWANE'){const marker=jobPartTechnicalDescription(x.cloud_id);let exists=db.prepare("SELECT id FROM order_items WHERE order_id=? AND technical_description=? ORDER BY id DESC LIMIT 1").get(x.order_id,marker);if(!exists){exists=db.prepare("SELECT id FROM order_items WHERE order_id=? AND kind='CZESC' AND name=? AND ABS(qty-?)<0.0001 AND ABS(unit_cost-?)<0.0001 ORDER BY id DESC LIMIT 1").get(x.order_id,x.name,x.qty,x.unit_cost);if(exists)db.prepare('UPDATE order_items SET technical_description=? WHERE id=?').run(marker,exists.id)}if(!exists){db.prepare("INSERT INTO order_items(order_id,kind,name,qty,unit_cost,unit_price,part_no,oe_number,supplier,notes,technical_description) VALUES (?,'CZESC',?,?,?,?,?,?,?,?,?)").run(x.order_id,x.name,x.qty,x.unit_cost,x.unit_price,x.part_no||'',x.oe_number||'',(db.prepare('SELECT name FROM suppliers WHERE id=?').get(x.supplier_id)||{}).name||x.supplier_name||'',[x.external_order_no&&`Zamówienie ${x.external_order_no}`,x.brand&&`Producent: ${x.brand}`,x.barcode&&`EAN: ${x.barcode}`].filter(Boolean).join(' · '),marker);syncOrderItemTotals(x.order_id)}}return true});return tx()})
ipcMain.handle('jobParts:batchStatus',(_,{ids,status,externalOrderNo='',expectedAt=null})=>{const db=getDb();const clean=[...new Set((ids||[]).map(Number).filter(Boolean))];if(!clean.length)return{ok:false,count:0};const tx=db.transaction(()=>{const placeholders=clean.map(()=>'?').join(',');const orderIds=[...new Set(db.prepare(`SELECT order_id FROM job_part_orders WHERE id IN (${placeholders})`).all(...clean).map(x=>x.order_id))];for(const orderId of orderIds)requireEditableOrder(db,orderId);for(const id of clean){db.prepare(`UPDATE job_part_orders SET status=?,external_order_no=CASE WHEN ?!='' THEN ? ELSE external_order_no END,expected_at=COALESCE(?,expected_at),ordered_at=CASE WHEN ?='ZAMOWIONE' AND ordered_at IS NULL THEN CURRENT_TIMESTAMP ELSE ordered_at END,received_at=CASE WHEN ?='ODEBRANE' THEN CURRENT_TIMESTAMP ELSE received_at END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(status,externalOrderNo,externalOrderNo,expectedAt,status,status,id)}});tx();return{ok:true,count:clean.length}})
...
ipcMain.handle('inventory:remove',(_,id)=>deleteInventoryPart(getDb(),id))
...
ipcMain.handle('qc:set',(_,{orderId,key,label,checked,note=''})=>{const db=getDb();requireEditableOrder(db,orderId);db.prepare(`INSERT INTO order_qc(order_id,check_key,label,checked,note,checked_at) VALUES (?,?,?,?,?,CASE WHEN ? THEN CURRENT_TIMESTAMP ELSE NULL END) ON CONFLICT(order_id,check_key) DO UPDATE SET label=excluded.label,checked=excluded.checked,note=excluded.note,checked_at=CASE WHEN excluded.checked=1 THEN CURRENT_TIMESTAMP ELSE NULL END`).run(orderId,key,label,checked?1:0,note,checked?1:0);syncCloseoutAutomation(orderId);return true})
