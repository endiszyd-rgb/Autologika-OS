...
ipcMain.handle('inventory:issue',(_,{inventoryPartId,orderId,qty})=>issueInventoryPart(getDb(),{inventoryPartId,orderId,qty}))
...
ipcMain.handle('jobParts:update',(_,{id,data})=>{const db=getDb();let price=+data.unit_price||0;if(!price&&+data.unit_cost>0)price=Math.round((+data.unit_cost)*(1+partMarkup(+data.unit_cost))*100)/100;db.prepare(`UPDATE job_part_orders SET supplier_id=?,part_no=?,oe_number=?,inventory_part_id=?,name=?,qty=?,unit_cost=?,unit_price=?,external_order_no=?,expected_at=?,notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(data.supplier_id||null,data.part_no||'',data.oe_number||'',data.inventory_part_id||null,data.name||'',+data.qty||1,+data.unit_cost||0,price,data.external_order_no||'',data.expected_at||null,data.notes||'',id);return true})
