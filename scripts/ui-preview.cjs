...
app.on('browser-window-created', (_, win) => {
  ...
  win.webContents.once('did-finish-load', async () => {
    try {
      ...
      await win.webContents.executeJavaScript(`window.autologika.jobParts.batchStatus([${Number(catalogJobPart.id)}],'ZAMONTOWANE')`)
      assert.equal(inventoryDb.prepare('SELECT COUNT(*) n FROM order_items WHERE technical_description=?').get(batchMarker).n,1)
      const batchItemId=inventoryDb.prepare('SELECT id FROM order_items WHERE technical_description=?').get(batchMarker).id
      await win.webContents.executeJavaScript(`window.autologika.items.remove(${Number(batchItemId)})`)
      ...
      console.log('ORDER_PART_BATCH_INSTALL','group installation billed every part once and removing billing restored the received status')
